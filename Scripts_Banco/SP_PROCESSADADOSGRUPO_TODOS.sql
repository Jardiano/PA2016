	     ...
	     DECLARE 					 
							 ...
							 FROM (
								SELECT
								   ...
								   (((SELECT TRUNC(((SUM (D.QUANTFATURAMENTO)/(SELECT SUM (B.QUANTFATURAMENTO) FROM CARGA_DADOS B INNER JOIN ITEM H ON B.ITEM_IDITEM = H.ID_ITEM
									  WHERE H.CLASSE_IDTIPO = E.CLASSE_IDTIPO AND B.DT_REALIZADA >= (SELECT TRUNC(ADD_MONTHS(MAX(DT_REALIZADA),-VALORPARAMETRO+1),'MONTH') AS DIA FROM CARGA_DADOS)))*100),2)  
									  FROM CARGA_DADOS D 
			...
			CURSOR LISTA02 IS (SELECT IDCLASSE,NOMECLASSE,QTDTOTCLIENTEMES,TOTALCLIENTE,VARIACAOCLI,TOTALFATURAMENTO,VARIACAOFAT,
							   ...
							   FROM (SELECT
								   ...
								   TRUNC((TRUNC
								   (((SELECT TRUNC(((SUM (D.QUANTFATURAMENTO)/(SELECT SUM (B.QUANTFATURAMENTO) FROM CARGA_DADOS B INNER JOIN ITEM H ON B.ITEM_IDITEM = H.ID_ITEM
									  WHERE H.CLASSE_IDTIPO = E.CLASSE_IDTIPO AND B.DT_REALIZADA >= (SELECT TRUNC(ADD_MONTHS(MAX(DT_REALIZADA),-VALORPARAMETRO+1),'MONTH') AS DIA FROM CARGA_DADOS)
